...
import { locals } from "@/plugins/background/locals";
...
const Settings = () => {
  ...
  useEffect(() => {
    if (!i18n.hasResourceBundle(i18n.language, manifest.id)) {
    }
  }, [i18n.language]);
  ...
  return (
    <div className='flex flex-col gap-4 pl-4'>
      <div className='flex flex-col gap-2'>
        <Label htmlFor='bg-image-url'>{t(`${manifest.id}:imageUrl`, 'Image URL')}</Label>
        <Input
          id='bg-image-url'
          onChange={(e) =>
      ...
      <div className='flex items-center gap-4 mt-2'>
        <div className='flex-1'>
          <Label className='pl-1 sm:pl-2' htmlFor='bg-opacity'>
            {t(`${manifest.id}:opacity`, 'Opacity')} ({current.opacity}%)
          </Label>
          <Slider
      ...
      <div className='flex items-center gap-4 mt-2'>
        <div className='flex-1'>
          <Label className='pl-1 sm:pl-2' htmlFor='bg-blur'>
            {t(`${manifest.id}:blur`, 'Blur')} ({current.blur}px)
          </Label>
          <Slider
            className='w-full mt-2'
            id='bg-blur'
      ...
      <div className='flex items-center justify-between mt-2 pr-2'>
        <div className='space-y-1'>
          <Label className='pl-1 sm:pl-2' htmlFor='bg-include-sidebar'>
            {t(`${manifest.id}:includeSidebar`, 'Include sidebar')}
          </Label>
        </div>
      ...
      <div className='flex gap-2 mt-2'>
        ...
        <Button
          ...
          variant='outline'>
          {t(`${manifest.id}:chooseImage`, 'Choose Image')}
        </Button>
      </div>
    </div>
  );
...
const Render = () => {
  ...
  const style = useMemo<React.CSSProperties>(() => {
    const chosen = current.sourceType === "local" ? current.imageData : current.imageUrl;
